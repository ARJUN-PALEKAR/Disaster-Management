...
const EnhancedMapComponent = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    ...
    // Function to create route line
    const createRoute = (start: L.LatLng, end: L.LatLng, parkName: string) => {
      ...
      // Create a simple straight line route (in real emergency, this would be acceptable)
      ...
      currentRouteRef.current = routeLine
      ...
      L.popup()
        ...
        .setContent(`
          <div style="text-align: center; padding: 5px;">
            <b>Route to ${parkName}</b><br/>
            <small>Distance: ~${distance} km</small><br/>
            <small style="color: #666;">Emergency Route (Direct Path)</small>
          </div>
        `)
        .openOn(map)
    ...
    parks.forEach((park) => {
      const marker = L.marker([park.coords[0], park.coords[1]], { icon: emergencyIcon })
        ...
        .bindPopup(`
          <div style="text-align: center; padding: 8px; min-width: 200px;">
            <b style="color: #ef4444;">${park.name}</b><br/>
            <small style="color: #666;">Emergency Recovery Center</small><br/>
            <button id="route-btn-${park.coords[0]}-${park.coords[1]}" 
      ...
      marker.on("popupopen", () => {
        const button = document.getElementById(`route-btn-${park.coords[0]}-${park.coords[1]}`)
        if (button) {
          button.onclick = () => {
            if (userMarkerRef.current) {
              createRoute(userMarkerRef.current.getLatLng(), L.latLng(park.coords[0], park.coords[1]), park.name)
            } else {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
          ...
          const userMarker = L.circleMarker([lat, lon], {
            ...
            .bindPopup(`
              <div style="text-align: center; padding: 5px;">
                ...
                <small>Click any emergency center for directions</small>
              </div>
            `)
        ...
        (error) => {
          ...
          // Show Tokyo center if geolocation fails
    ...
    // Add map controls
    const clearRouteButton = L.control({ position: "topright" })
    clearRouteButton.onAdd = () => {
      const div = L.DomUtil.create("div", "clear-route-control")
      div.innerHTML = `
        <button style="padding: 8px 12px; background: #ef4444; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 12px;">
          🗑️ Clear Route
        </button>
      `
      div.onclick = () => {
...
EnhancedMapComponent.displayName = "EnhancedMapComponent"
